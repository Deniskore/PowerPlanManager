...
bool Utils::IsFullScreen(HWND window) {
...
bool Utils::CheckAdministratorToken(PSID *ppAdministratorsGroup,
                                    BOOL *pfIsRunAsAdmin) {
  ...
  if (!CheckTokenMembership(nullptr, *ppAdministratorsGroup, pfIsRunAsAdmin)) {
    return false;
  }
...
void Utils::ManageAutoStartup(bool enableAutoStartup) {
  if (!IsAdministrator()) {
    ...
    if (!szPath.empty()) {
      ...
      if (!ShellExecuteEx(&sei)) {
        ...
        if (dwError == ERROR_CANCELLED) {
          MessageBox(nullptr, L"The user cancelled elevation", L"Error", MB_OK);
        }
      }
    }
  } else {
...
bool Utils::AddToStartup(const std::wstring &programPath) {
  HKEY hkey = nullptr;
  LONG result = RegOpenKeyEx(
      HKEY_CURRENT_USER, LR"(SOFTWARE\Microsoft\Windows\CurrentVersion\Run)", 0,
